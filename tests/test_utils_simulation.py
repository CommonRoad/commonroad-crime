...
from commonroad_crime.utility.simulation import (
    SimulationLong,
    SimulationLat,
    Maneuver,
    SimulationLongMonteCarlo,
    SimulationLatMonteCarlo,
...
class TestSimulation(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.config.update()
        self.ego_vehicle = self.config.scenario.obstacle_by_id(
            self.config.vehicle.ego_id
        )
    ...
    def test_simulation_long(self):
        ...
        self.assertEqual(
            simulated_state1[-1].time_step, self.ego_vehicle.prediction.final_time_step
        )
    ...
    def test_simulation_long_mc(self):
        sim_stat_list_total = []
        sim_long = SimulationLongMonteCarlo(
            Maneuver.STOPMC, self.ego_vehicle, self.config
        )
    ...
    def test_simulation_lat(self):
        ...
        sim_lat_right = SimulationLat(
            Maneuver.STEERRIGHT, self.ego_vehicle, self.config
        )
        simulated_state2 = sim_lat_right.simulate_state_list(10)
        self.config.time.steer_width = 2
        sim_lat_left_2 = SimulationLat(
            Maneuver.STEERLEFT, self.ego_vehicle, self.config
        )
        simulated_state3 = sim_lat_left_2.simulate_state_list(0)

        # overtaking
        sim_lat_left = SimulationLat(
            Maneuver.OVERTAKELEFT, self.ego_vehicle, self.config
        )
        simulated_state4 = sim_lat_left.simulate_state_list(5)
        sim_lat_right = SimulationLat(
            Maneuver.OVERTAKERIGHT, self.ego_vehicle, self.config
        )
        ...
        self.assertEqual(
            simulated_state2[-1].time_step, self.ego_vehicle.prediction.final_time_step
        )
        self.assertEqual(
            simulated_state3[-1].time_step, self.ego_vehicle.prediction.final_time_step
        )
        self.assertEqual(
            simulated_state4[-1].time_step, self.ego_vehicle.prediction.final_time_step
        )
        self.assertEqual(
            simulated_state5[-1].time_step, self.ego_vehicle.prediction.final_time_step
        )

        for sim_state_list in [
            simulated_state1,
            simulated_state2,
            simulated_state3,
            simulated_state4,
    ...
    def test_simulation_lat_mc(self):
        sim_stat_list_total = []
        self.config.time.steer_width = 2
        sim_lat_left = SimulationLatMonteCarlo(
            Maneuver.LANECHANGEMC, self.ego_vehicle, self.config
        )
        for i in range(10):
    ...
    def test_simulation_random(self):
        sim_stat_list_total = []
        sim_lat_left = SimulationRandoMonteCarlo(
            Maneuver.RANDOMMC, self.ego_vehicle, self.config
        )
        for i in range(10):
            sim_stat_list_total.append(sim_lat_left.simulate_state_list(0))
            for sim_state_list in sim_stat_list_total:
            ...
            utils_vis.save_fig(
                "test_simulate_mc_random", self.config.general.path_output, 0
            )
