...
from commonroad_crime.measure import BTN, CI, PSD, DA
import time
...
def main():
    ...
    utils_batch.run_parallel(scenario_path_4p, measures=[DA, PSD], verbose=False)
    end_par = time.time()
    print("Testing run_sequential()...")
    utils_batch.run_sequential(scenario_path_4s, measures=[CI], verbose=False)
    end_seq = time.time()
    par_t = end_par - start
