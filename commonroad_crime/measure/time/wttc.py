...
class WTTC(CriMeBase):
    ...
    def visualize(self, figsize: tuple = (25, 15)):
        ...
        if isinstance(self.other_vehicle, StaticObstacle):
            r_2 = 0
        r_v1, _ = utils_sol.compute_disc_radius_and_distance(
            self.ego_vehicle.obstacle_shape.length,
            self.ego_vehicle.obstacle_shape.width,
        )
        r_v2, _ = utils_sol.compute_disc_radius_and_distance(
            self.other_vehicle.obstacle_shape.length,
        ...
        new_x_2 = (
            self.other_vehicle.state_at_time(self.time_step).position[0]
            + self.value * self.other_vehicle.state_at_time(self.time_step).velocity
        )
        new_y_2 = (
            self.other_vehicle.state_at_time(self.time_step).position[1]
        ...
        utils_vis.draw_circle(
            self.rnd, np.array([new_x_1, new_y_1]), r_v1 + r_1, color=TUMcolor.TUMblue
        )
        utils_vis.draw_circle(
            self.rnd, np.array([new_x_1, new_y_1]), r_v1, color=TUMcolor.TUMdarkblue
        )
        utils_vis.draw_circle(
            self.rnd,
            np.array([new_x_2, new_y_2]),
            r_v2 + r_2,
            color=TUMcolor.TUMlightgray,
        )
        utils_vis.draw_circle(
            self.rnd, np.array([new_x_2, new_y_2]), r_v2, color=TUMcolor.TUMdarkgray
        )
