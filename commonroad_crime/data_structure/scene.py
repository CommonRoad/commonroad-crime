...
class Scene(Scenario):
    ...
    def static_obstacles(self) -> List[StaticObstacle]:
        ...
        return [
            stat_obs
            for stat_obs in self._scenario.static_obstacles
            if stat_obs.state_at_time(self.time_step) is not None
        ]
    ...
    def dynamic_obstacles(self) -> List[DynamicObstacle]:
        ...
        return [
            dyn_obs
            for dyn_obs in self._scenario.dynamic_obstacles
            if dyn_obs.state_at_time(self.time_step) is not None
        ]
